...
from app.services.credit_compare import build_academic_map, compute_student_progress, report_to_dict
...
@router.get("/me/map", summary="当前学生学业地图")
def my_academic_map(db: DbSession, current: CurrentUser):
    student = db.scalar(select(Student).where(Student.user_id == current.id))
    if not student:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "未关联学生信息")
    return build_academic_map(db, student)
...
@router.get("/{student_id}/map", summary="指定学生学业地图", dependencies=[Depends(require_staff)])
def student_academic_map(student_id: int, db: DbSession):
    student = get_or_404(db, Student, student_id, "学生")
    return build_academic_map(db, student)
